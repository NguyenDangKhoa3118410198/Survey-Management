   ...
   useEffect(() => {
      if (userDetail) {
         ...
         const processedAddresses = addresses?.map((address) => ({
            city: address?.city?.includes('-')
               ? address.city.split('-')[1]
               : address.city,
            district: address?.district?.includes('-')
               ? address.district.split('-')[1]
               : address.district,
            ward: address?.ward?.includes('-')
               ? address.ward.split('-')[1]
               : address.ward,
         }));
   ...
   const handleSubmit = (values: IUser) => {
      try {
         ...
         if (deletedAvatar && !values?.avatar?.file?.response?.physicalPath) {
            ...
            pathImg =
               values?.avatar?.file?.response?.physicalPath ??
               userDetail?.avatar;
         }

         const formattedBirthDate = dayjs(values.birthDate).format(
            'DD/MM/YYYY'
         );
               ...
               rules={[
                  {
                     pattern: /^0[1-9]\d{8}$/,
                     message:
                        'Số điện thoại nên bắt đầu với 0 và bao gồm 10 số',
                  },
               ]}
            >
